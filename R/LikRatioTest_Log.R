...
# initial parameter values for competitive (th0E2) and non competitive (th1E2) models (Log case)
th0E2 <- c(6.0645, 3.2799, 3.3153)
th1E2 <- c(12.0125, 8.5359, 5.6638)
...
#-------------------------------------------------------------------------------------------------
# zeros in the data are replaced by epsilon=0.02
...
generate_y_Example2 <- function(xi, model, th, sig) {
  ...
  # Generates data from one of the two models from Atkinson 2012
  ...
  if (model == 0) y <- log(f0(xi[, 1], xi[, 2], th)) + rnorm(nrow(xi), sd = sig)
  if (model == 1) y <- log(f1(xi[, 1], xi[, 2], th)) + rnorm(nrow(xi), sd = sig)
...
decide_Example2 <- function(xi, y, th0i, th1i, model) 
{
  ...
  l0 <- function(x, xi, y){
    ((-n/2)*log(2*pi*(sigh0^2)))-(1/2*(sigh0^2))*sum((y - log((x[1] * xi[, 1] / (x[2] * (1 + xi[, 2]/x[3]) + xi[, 1]))) )^2) 
  }
  
  # log likelihood function for model 1
  l1 <- function(x, xi, y){
    ((-n/2)*log(2*pi*(sigh1^2)))-(1/2*(sigh1^2))*sum((y - log((x[1] * xi[, 1] / ((x[2] + xi[, 1]) * (1 + xi[, 2]/x[3])))) )^2)
  }
...
t0<-decide_Example2(xmat,log(basia[,1]),th0E2,th1E2,0)$T.stat
t1<-decide_Example2(xmat,log(basia[,1]),th0E2,th1E2,1)$T.stat
