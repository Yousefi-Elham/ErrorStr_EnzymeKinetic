# computation of the T-optimum design A1, presented in the table 5 in the log case 
...
theta1 <- c(12.0125, 8.5359, 5.6638,0) #nu=0, non competitive log model is fixed

theta2.start <- c(6.0645, 3.2799, 3.3153) #competitive log model is fitted
lambdac=1
...
while(iter< max.iter){
  ...
  Tp.theta=function(par){
    ...
    eta1=log((theta1[1]*xstart[1,])/ ((theta1[2]*(1+(xstart[2,]/theta1[3]))) + xstart[1,]*(1+(((1-theta1[4])*xstart[2,])/theta1[3])))) #11
    eta2=log((theta21*xstart[1,])/ ((theta22*(1+(xstart[2,]/theta23)))+xstart[1,]*(1+(((1-lambdac)*xstart[2,])/theta23)))) #10
  ...
  #                                                        X.HAT
  #-----------------------------------------------------------------------------------------------------------------------
  #delta_0 formulation in T-optimality definition (lack of fit sum of squares when the non competitive log model is fixed)
  ...
  DELTA1=DELTA(theta1,c(theta2.hat,lambdac))
  #----------------------------------------------------
  ...
  psi=function(xest11,xest22){
    ...
    eta22=log((theta21.hat*xest1)/ ((theta22.hat*(1+(xest2/theta23.hat))) + xest1*(1+(((1-lambdac)*xest2)/theta23.hat))))
    eta33=(eta11-eta22)^2
    eta33/DELTA1
  }
