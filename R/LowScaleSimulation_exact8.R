## Section 4.3, A simulation study of discriminating designs, subsection 4.3.1 Exact designs, N = 8
...
#N=8 number of exact design


des.A1 <- matrix(c(30.00,    0,
                   0.02,   60,
                   0.02,   60,
                   0.02,   60,
                   30.00,   60,
...
#---------------------------------------------------------------------------------------------------
# Classification performance of des.A3
set.seed(123456789)
res.A3 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.A3 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.A3 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.A3 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.A3 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.A3, 0, th0.pert, csig*sigh)
        
        res.A3[ic, im] <- res.A3[ic, im] + sim0$percent[1]
        C0.A3[i, ic] <-sim0$worse
        Neg.C0.A3[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.A3, 1, th1.pert, csig*sigh)
        
        res.A3[ic, im] <- res.A3[ic, im] + sim1$percent[2]
        C1.A3[i, ic] <-sim1$worse
        Neg.C1.A3[i, ic] <- sim1$neg
      }
    }
  }
}
...
#---------------------------------------------------------------------------------------------------
# Classification performance of des.delta2
set.seed(123456789)
res.delta2 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.delta2 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.delta2 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.delta2 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.delta2 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.delta2, 0, th0.pert, csig*sigh)
        
        res.delta2[ic, im] <- res.delta2[ic, im] + sim0$percent[1]
        C0.delta2[i, ic] <-sim0$worse
        Neg.C0.delta2[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.delta2, 1, th1.pert, csig*sigh)
        
        res.delta2[ic, im] <- res.delta2[ic, im] + sim1$percent[2]
        C1.delta2[i, ic] <-sim1$worse
        Neg.C1.delta2[i, ic] <- sim1$neg
      }
    }
  }
}
res.delta2 <- res.delta2 / N1

#---------------------------------------------------------------------------------------------------
# Classification performance of des.delta3
set.seed(123456789)
res.delta3 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.delta3 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.delta3 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.delta3 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.delta3 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.delta3, 0, th0.pert, csig*sigh)
        
        res.delta3[ic, im] <- res.delta3[ic, im] + sim0$percent[1]
        C0.delta3[i, ic] <-sim0$worse
        Neg.C0.delta3[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.delta3, 1, th1.pert, csig*sigh)
        
        res.delta3[ic, im] <- res.delta3[ic, im] + sim1$percent[2]
        C1.delta3[i, ic] <-sim1$worse
        Neg.C1.delta3[i, ic] <- sim1$neg
      }
    }
  }
}
res.delta3 <- res.delta3 / N1
...
Table3 <- rbind( as.vector(t(res.A1)), as.vector(t(res.A2)),
                 as.vector(t(res.A4)), as.vector(t(res.delta1)),
                 as.vector(t(res.delta5)),as.vector(t(res.delta52)),
                 as.vector(t(res.delta6)), as.vector(t(res.delta61)),
                 as.vector(t(res.delta62)),as.vector(t(res.delta72)) )

print(Table3)

#--------------------------------------------------------------------------------------
# we have finally reported average values of hit rates when both models contribute equally in simulations
# related to Table 7, part N = 8
