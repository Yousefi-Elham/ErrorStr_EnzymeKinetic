## Section 4.3, A simulation study of discriminating designs, subsection 4.3.1 Exact designs, N = 6
...
################################################################################################
#N=6 number of exact design


des.A1 <- matrix(c(rep(c(30.00, 0),1),rep(c(0.02, 60),2),
                   rep(c(30.00, 60),3)), ncol = 2, byrow = TRUE)


des.A2 <- matrix(c(rep(c(0.02, 0),1),rep(c(30.00, 0),1),
                   rep(c(0.02, 60),2),rep(c(30.00, 60),2)), ncol = 2, byrow = TRUE)

#des.A3=des.A2
#des.A3 <- matrix(c(rep(c(0.02, 0),1),rep(c(30.00, 0),1),
#                   rep(c(0.02, 60),2),rep(c(30.00, 60),2)), ncol = 2, byrow = TRUE)


des.A4 <- matrix(c(rep(c(0.02, 0),1),rep(c(30.00, 0),2),
                   rep(c(0.02, 60),2),rep(c(30.00, 60),1)), ncol = 2, byrow = TRUE)

#-----------------------------------------------------------------------------------------------
#des.A1=des.delta1=des.delta2=des.delta3 <- matrix(c(rep(c(30.00, 0),1),rep(c(0.02, 60),2),
#                      rep(c(30.00, 60),3)), ncol = 2, byrow = TRUE)

des.delta4 <- matrix(c(rep(c(0.02, 0),1),rep(c(30.00, 0),2),
                       rep(c(0.02, 60),1),rep(c(30.00, 60),2)), ncol = 2, byrow = TRUE)


des.delta5 <- matrix(c(rep(c(1, 0),1),rep(c(30.00, 0),2),
                       rep(c(0.02, 60),1),rep(c(30.00, 60),2)), ncol = 2, byrow = TRUE)


des.delta51 <- matrix(c(rep(c(1, 0),1),rep(c(30.00, 0),1),
                        rep(c(0.02, 60),2),rep(c(30.00, 60),2)), ncol = 2, byrow = TRUE)


#des.delta52=des.delta51 <- matrix(c(rep(c(1, 0),1),rep(c(30.00, 0),1),
#                        rep(c(0.02, 60),2),rep(c(30.00, 60),2)), ncol = 2, byrow = TRUE)


des.delta6  <- matrix(c(4,    0,
                        30,    3,
                        30,    3,
                        5,   60,
                        30,   60,
                        30,   60) , ncol = 2, byrow = TRUE)
...
des.delta7 <- matrix(c(0.02,    0,
                       3.00,    0,
                       4.00,    0,
                       30.00,    2,
                       6.00,   48,
                       30.00,   57) , ncol = 2, byrow = TRUE)

des.delta71 <- matrix(c(0.02,    0,
                        3.00,    0,
                        5.00,    0,
                        30.00,    2,
                        7.00,   60,
...
des.delta72 <- matrix(c(0.02,    0,
                        3.00 ,   0,
                        4.00,    0,
                        30.00,    2,
                        30.00,   59,
                        6.00 ,  60) , ncol = 2, byrow = TRUE)
...
#-------------------------------------------------------------------------------------------------
# Classification performance of des.delta51 
set.seed(123456789)
res.delta51 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.delta51 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.delta51 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.delta51 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.delta51 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.delta51, 0, th0.pert, csig*sigh)
        
        res.delta51[ic, im] <- res.delta51[ic, im] + sim0$percent[1]
        C0.delta51[i, ic] <-sim0$worse
        Neg.C0.delta51[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.delta51, 1, th1.pert, csig*sigh)
        
        res.delta51[ic, im] <- res.delta51[ic, im] + sim1$percent[2]
        C1.delta51[i, ic] <-sim1$worse
        Neg.C1.delta51[i, ic] <- sim1$neg
      }
    }
  }
}
res.delta51 <- res.delta51 / N1
...
#-------------------------------------------------------------------------------------------------
# Classification performance of des.delta7 
set.seed(123456789)
res.delta7 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.delta7 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.delta7 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.delta7 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.delta7 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.delta7, 0, th0.pert, csig*sigh)
        
        res.delta7[ic, im] <- res.delta7[ic, im] + sim0$percent[1]
        C0.delta7[i, ic] <-sim0$worse
        Neg.C0.delta7[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.delta7, 1, th1.pert, csig*sigh)
        
        res.delta7[ic, im] <- res.delta7[ic, im] + sim1$percent[2]
        C1.delta7[i, ic] <-sim1$worse
        Neg.C1.delta7[i, ic] <- sim1$neg
      }
    }
  }
}
res.delta7 <- res.delta7 / N1

#-------------------------------------------------------------------------------------------------
# Classification performance of des.delta71 
set.seed(123456789)
res.delta71 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.delta71 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.delta71 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.delta71 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.delta71 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.delta71, 0, th0.pert, csig*sigh)
        
        res.delta71[ic, im] <- res.delta71[ic, im] + sim0$percent[1]
        C0.delta71[i, ic] <-sim0$worse
        Neg.C0.delta71[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.delta71, 1, th1.pert, csig*sigh)
        
        res.delta71[ic, im] <- res.delta71[ic, im] + sim1$percent[2]
        C1.delta71[i, ic] <-sim1$worse
        Neg.C1.delta71[i, ic] <- sim1$neg
      }
    }
  }
}
res.delta71 <- res.delta71 / N1

#-------------------------------------------------------------------------------------------------
# Classification performance of des.delta72
set.seed(123456789)
res.delta72 <- matrix(0, nrow = length(c.vec), ncol = 2)
C0.delta72 <- matrix(0, nrow = N1, ncol = length(c.vec))
C1.delta72 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C0.delta72 <- matrix(0, nrow = N1, ncol = length(c.vec))
Neg.C1.delta72 <- matrix(0, nrow = N1, ncol = length(c.vec))
  ...
  for (im in 1:2) {
    ...
    for (i in 1:N1) {
      ...
      if (im == 1) {
        ...
        sim0<-simulate_LN_Example2(Nexp, des.delta72, 0, th0.pert, csig*sigh)
        
        res.delta72[ic, im] <- res.delta72[ic, im] + sim0$percent[1]
        C0.delta72[i, ic] <-sim0$worse
        Neg.C0.delta72[i, ic] <- sim0$neg
      ...
      if (im == 2) {
        ...
        sim1<-simulate_LN_Example2(Nexp, des.delta72, 1, th1.pert, csig*sigh)
        
        res.delta72[ic, im] <- res.delta72[ic, im] + sim1$percent[2]
        C1.delta72[i, ic] <-sim1$worse
        Neg.C1.delta72[i, ic] <- sim1$neg
      }
    }
  }
}
res.delta72 <- res.delta72 / N1
...
Table3 <- rbind( as.vector(t(res.A1)), as.vector(t(res.A2)),
                 as.vector(t(res.A4)),as.vector(t(res.delta4)),
                 as.vector(t(res.delta5)),as.vector(t(res.delta51)),
                 as.vector(t(res.delta6)),
                 as.vector(t(res.delta61)),as.vector(t(res.delta62)),
                 as.vector(t(res.delta7)),as.vector(t(res.delta71)),
                 as.vector(t(res.delta72)) )

print(Table3)

#-----------------------------------------------------
# we have finally reported average values of hit rates when both models contribute equally in simulations
# related to Table 7, part N = 6
